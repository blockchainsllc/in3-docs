...
```sh cmake_minimum_required(VERSION 3.4.1)
...
foreach(module core verifier/eth1/nano verifier/eth1/evm verifier/eth1/basic verifier/eth1/full bindings/java third-party/crypto third-party/tommath api/eth1) file(MAKE_DIRECTORY in3-core/src/${module}/outputs) add_subdirectory( in3-core/src/${module} in3-core/src/${module}/outputs) endforeach() ```

1. clone [in3-core](https://git.slock.it/in3/c/in3-core.git) into the `app`-folder or use this script to clone and update incubed:

```sh #!/usr/bin/env sh

IN3_SRC=[git@github.com](mailto:git@github.com):SlockItEarlyAccess/in3-core.git cd app

if [ -d in3-core ]; then cd in3-core git pull cd .. else git clone $IN3_SRC fi

1. Use methods available in app/src/main/java/in3/IN3.java from android activity to access IN3 functions.
